...
@app.get("/health")
async def health():
    """Health check endpoint - Verify both direct API and LangChain"""
    try:
        ...
        langchain_status = llm is not None
        
        return {
            "status": "healthy" if (ollama_status and langchain_status) else "degraded",
            "ollama": "connected" if ollama_status else "disconnected",
            "langchain": "initialized" if langchain_status else "error",
            "model": LLM_MODEL,
